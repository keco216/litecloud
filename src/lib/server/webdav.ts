...
import { db } from './db';
import { users, files, shares, fileMetadata } from './db/schema';
...
async function handlePropfind(user: DavUser, path: string, request: Request): Promise<Response> {
	...
	if (path === '/') {
		...
		if (depth > 0) {
			const items = db.select().from(files).where(and(eq(files.userId, user.id), eq(files.path, '/'))).all();
		...
		const item = db
			.select()
			.from(files)
			.where(and(eq(files.userId, user.id), eq(files.path, parent), eq(files.name, name)))
			.get();
		...
		if (item.isFolder && depth > 0) {
			...
			const children = db.select().from(files).where(and(eq(files.userId, user.id), eq(files.path, folderPath))).all();
...
async function handleDelete(user: DavUser, path: string): Promise<Response> {
	...
	if (item.isFolder) {
		const folderPath = parent === '/' ? `/${name}` : `${parent}/${name}`;
		const all = db.select().from(files).where(eq(files.userId, user.id)).all();
